/**
 * One-shot: parse the interpreter en-es list into interpreter-glossary.json.
 * Run: node src/experiments/trial-soniox-x/_build-en-es-glossary.mjs
 */
...
const rawPath = path.join(dir, "_en-es-medical-raw.txt");
...
for (const line of raw.split(/\n/)) {
  ...
  const esRaw = line.slice(idx + 1);
  const es = primaryEs(esRaw);
  if (!es) continue;
  ...
  const esAbbr = [...esRaw.matchAll(/\b([A-Z]{2,8})\b/g)].map((m) => m[1]);
  for (const abbr of esAbbr) {
    if (abbr === "OR" || seen.has(abbr.toLowerCase())) continue;
    seen.set(abbr.toLowerCase(), { en: abbr, es });
    rows.push({ en: abbr, es });
  }
}

const existing = JSON.parse(fs.readFileSync(outPath, "utf8"));
existing.pairs["en-es"] = rows;
fs.writeFileSync(outPath, `${JSON.stringify(existing, null, 2)}\n`);
console.log(`en-es rows: ${rows.length} unique-ish entries`);
