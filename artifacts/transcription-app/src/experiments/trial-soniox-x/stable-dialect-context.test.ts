...
describe("stable dialect pins", () => {
  ...
  it("puts the selected pair in general context and does not dump all 60 languages", () => {
    ...
    expect(ctx.general?.some((row) => /Never lock onto the first language/i.test(row.value))).toBe(true);
    expect(ctx.general?.some((row) => /beginning, middle, and end/i.test(row.value))).toBe(true);
