...
export default function AdminMarketingDemo() {
  ...
  // Soft active-row highlight: steady while a segment grows; moves only when the active row changes.
  useEffect(() => {
    ...
    const applyActiveRow = () => {
      ...
      if (nextActive !== activeRowRef.current) {
      ...
      } else if (nextActive && !nextActive.classList.contains("demo-row-active")) {
        nextActive.classList.add("demo-row-active");
      }
    ...
    const scheduleApply = () => {
      ...
      rafId = requestAnimationFrame(applyActiveRow);
    };
    ...
    >
      ...
      >
        ...
        >
          <span
            className={cn(
              "text-[9px] font-semibold uppercase tracking-[0.16em] truncate text-center",
