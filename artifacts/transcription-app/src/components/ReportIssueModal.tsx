...
export function ReportIssueModal({ isOpen, onClose, defaultEmail = "" }: Props) {
  ...
  const reset = () => {
    setDesc(""); setError(null); setSent(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !desc.trim()) return;
    ...
    try {
      const res = await fetch("/api/support", {
        ...
        body: JSON.stringify({
          email:   email.trim(),
          subject: "[Translation Issue Report]",
          message: desc.trim(),
        }),
      });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-border">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">Report Translation Issue</h2>
          </div>
          <button
            onClick={handleClose}
            ...
            <div className="flex flex-col items-center text-center py-4 gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Thank you — our team will review the translation issue.
                </p>
              </div>
              <button
                onClick={handleClose}
            ...
            <form onSubmit={(e) => void handleSubmit(e)} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe the translation issue — e.g. wrong word, wrong language, missing text…"
                  required
                  minLength={10}
                  rows={4}
              ...
              <button
                type="submit"
                disabled={loading || !email.trim() || !desc.trim()}
                className="w-full h-9 rounded-lg bg-orange-500 text-white text-xs font-semibold hover:bg-orange-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
              >
                {loading ? (
