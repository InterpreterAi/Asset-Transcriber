...
export function DeleteAccountSection({
  ...
  return (
    <div className={cn(compact ? "pt-1" : "pt-2")}>
      <h2 className={cn("font-medium text-foreground", compact ? "text-sm" : "text-base")}>
        {compact ? "Close account" : "Delete account"}
      </h2>
      <p className={cn("text-muted-foreground mt-1", compact ? "text-[11px] leading-relaxed" : "text-sm")}>
        Permanently close your account. You will not be able to sign in again with this email.
        {hasPayPalSubscription ? " Any active PayPal subscription will be cancelled." : ""}
      </p>
      {!compact && (
        <p className="text-xs text-muted-foreground mt-1.5">
          Need help with your data? Contact Support from the workspace menu.
        </p>
      )}
        ...
        <AlertDialogTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              compact ? "mt-2 h-8 px-0 text-xs underline-offset-4 hover:underline" : "mt-3 h-9 px-0 text-sm underline-offset-4 hover:underline",
            )}
            type="button"
          >
